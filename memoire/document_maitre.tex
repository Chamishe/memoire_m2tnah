...
%%%% PACKAGES UTILISÉS %%%%
	...
	%%% Mise en forme %%%
		...
		% NB : Etant donné que nous avions ajouté le style de l'école à nos fichiers locaux dont je n'ai pas eu besoin de faire ce lien. 
	...
	%%% Packages personnels %%%
		...
		\usepackage{tabularx} % Insertion de tableaux avec des colonnes plus flexibles que tabular. 
		\usepackage{longtable} % Permet d'avoir des tableaux qui dépassent sur plusieurs pages. A noter qu'il n'est pas possible d'avoir une tableau tabularx ou longtable, il faut choisir.
...
% Transcription %
% Ces commandes ont été réalisée pour minimisée les fautes de frappes sur les renvois à mes annexes. Elles sont très répétitives et les fautes de frappes sont courantes %

	\newcommand{\PL}% Nom de la commande
		{Pascal Lebeurier} % Ce qu'affiche cette commande 
	
	\newcommand{\AH}{Arthur Hénaff}
	
	\newcommand{\DA}{la documentaliste anonymisée}

	\newcommand{\renvoientretien}[2]{\footnote{Voir entretien de #1 sur #2. Annexe \ref{ann:transcriptions}, \nameref{ann:transcriptions}, p. \pageref{ann:transcriptions}.}}
	% Créer une commande qui me crée une note de bas de page pour les transcriptions. Les arguments sont 1 : Nom de l'intervenant, 2 : Sujet abordé (car les entretiens sont découpés thématiquements), 3 : Label de l'annexe. 
	
	\newcommand{\renvoiannexe}[1]{\footnote{Voir Annexe \ref{#1}, \nameref{#1}, p. \pageref{#1}}.} % Même procédé pour une annexe simple pour éviter de réecrire plusieurs fois le label de l'annexe. 
...
\input{./partie2/introduction2.tex}
	...
	\chapter{Impact de l'hétérogénérité des données sur les pratiques des publics des institutions patrimoniales}
		...
		\input{./partie2/chapitre2/2_perimetres.tex}
		
		\section{Un problème de qualité}
		
		\input{./partie2/chapitre2/4_qualite.tex}
		
		\section{Nouveau mode de consommation des informations}
		
		\input{./partie2/chapitre2/1_consommation_donnees.tex}
		
\chapter*{Conclusion de la partie 1}

\input{./partie2/conclusion2.tex}
		
	
\part{De quel contexte sont issues ces données hétérogènes ?}

\input{./partie1/introduction1.tex}

\chapter{Un contexte social en mouvement}

	\section{Mise en place d'un cadre légal}
	
	\input{./partie1/chapitre1/1_mise_en_place_contexte_legal.tex} %Input: importer un fichier
	
	\section{Délimitation de la conservation}
	
	\input{./partie1/chapitre1/2_delimitation_conservation.tex}
	 
	\section{Impact des changements institutionnels : le facteur humain dans les données}
	
	\input{./partie1/chapitre1/3_impacts_institutionnels.tex}
	
	\section{Impact du contexte économique}
	
	\input{./partie1/chapitre1/4_impacts_economiques.tex}
...
\renewcommand{\addcontentsline}[3]{}
	...
	\input{./annexes/transcriptions.tex}
